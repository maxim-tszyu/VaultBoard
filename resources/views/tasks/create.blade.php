@php use App\Enums\Priority;use App\Models\Task;@endphp
<x-app-layout :$sidebarLinks>
	...
	<form action="{{ route('tasks.store') }}" method="POST" class="space-y-6">
		...
		<div>
			<label for="parent_task_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Parent
				Task</label>
			<select id="parent_task_id" name="parent_task_id"
					class="block w-full p-2.5 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 dark:focus:ring-primary-500 dark:focus:border-primary-500">
				<option selected disabled>Select parent task</option>
				@foreach(Task::all()->where('user_id', auth()->user()->id) as $task)
        ...
        <span
				...
				<div x-show="open" @click.away="open = false"
					...
					<div class="px-3 py-2 border-t">
						<a href="{{ route('categories.create') }}" class="text-blue-600">+ Create new category</a>
					</div>
				</div>
